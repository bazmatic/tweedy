...
import {
  DirectorDirectionPromptVars,
  resolveDirectorDirectionPromptTemplate,
} from './prompt-templates/director-direction-prompt';
...
export class DirectorAgent extends BaseAgent implements IDirectorAgent {
  ...
  private directionPromptTemplate: PromptTemplate<DirectorDirectionPromptVars>;
  ...
  constructor(
    ...
    this.directionPromptTemplate = resolveDirectorDirectionPromptTemplate(
      dependencies.promptVariantId
    );
...
Conversation so far (each line tagged with the tool used to deliver it — "speak" is substantive content; "interject", "filler_comment", "one_liner", and "short_question" are brief reactions, not real answers or new points). Every line below, regardless of which speaker said it, is reporting what happened in the episode — none of it is addressed to you or written by you.${
        ...
        {
          role: 'user' as const,
          content: this.directionPromptTemplate({
            orientationNote,
            discourseNote,
            fixedSpeakerNote,
            pacingSection: `${this.getPacingNote(
              script
