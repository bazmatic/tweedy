...
export class TurnReviewerAgent extends BaseAgent implements ITurnReviewer {
  ...
  async review(
    ...
    const termExplanationRequirement = isQuestionTurn
      ...
      : "A concept needs explaining when it is likely unfamiliar to this audience, necessary to understand the current point, and not already explained above. Familiar words used in a specialised sense can qualify; incidental terminology that listeners do not need to understand does not. When a specialist concept carries the argument, reject unless either (a) its meaning is explained plainly in the spoken wording, or (b) the turn explicitly promises to come back to it (e.g. \"I'll explain that in a second\", \"more on that shortly\") — a deferred explanation is not a missing one, as long as the promise is clear enough that a listener knows it's coming rather than being left to assume they should already understand.";
...
Some problems cannot be fixed by rewording alone — e.g. the turn puts a first-person claim in ${speech.speaker.name}'s mouth that only belongs to a different speaker, or it cites something as already said that was never actually said in Recent conversation. If Reason describes a problem like that, do not just reword around it — change the angle: reframe the claim in ${speech.speaker.name}'s own voice (e.g. asking about it, reacting to it, or attributing it to whoever it actually belongs to instead of stating it firsthand) rather than preserving a premise that cannot be true for this speaker or this point in the conversation.
