...
`AudioService` also supports a second, multispeaker pipeline for providers (`IMultispeakerVocalProvider`, e.g. `GoogleGeminiMultispeakerProvider`) that synthesize several dialogue turns in one call instead of one clip per turn. `checkMultispeakerEligibility` (`src/services/multispeaker-eligibility.ts`) activates it only when a script has exactly 2 speakers on the same multispeaker-capable voice provider — any other mix (3+ speakers, split providers) falls back to the per-clip pipeline, logging a warning if the fallback was caused by a provider mismatch. The script is split into provider-sized batches via `chunkTurns` (`src/providers/multispeaker-chunking.ts`), each batch synthesized as one audio file, and those files are concatenated through the same `AudioProcessor.concatenateAudio` path (with `isInterjection` always `false`, since overlap isn't reproducible for an atomic multi-turn blob). Per-turn timeline boundaries within each batch are approximated via silence detection (`splitChunkIntoTurns`, `src/providers/silence-turn-splitter.ts`), falling back to evenly dividing the batch's duration if the detected silence-gap count doesn't match the turn count. `wordTimestamps` are never populated for multispeaker-generated entries. `regenerateSpeech` in this mode re-synthesizes and re-splits the whole batch a speech belongs to, not just that one turn, since a batch is one atomic TTS call.
